...
public class CellContainer
{
    ...
    private bool ContainerHovered => Collision.BoxContainsCursor(LeftBorder, TopBorder, RightBorder, BottomBorder);

    // -- Initialization

    public CellContainer(int xPos, int yPos, int cellSize, int cellCount, int rowSize, int cellGap = 5, int padding = 6)
    {
    ...
    {
        if (Collision.BoxContainsCursor(cell.Position.X, cell.Position.Y, _cellSize, _cellSize))
            cell.Hovered();
    }
    ...
    public void Draw(Color? background = null, Color? cells = null, Color? border = null, Color? cellBorder = null)
    {
        ...
        foreach (var cell in _cells)
        {
            ...
            {
                Batcher.DrawString(cell.Label, new Vector2(cell.Position.X + 4, cell.Position.Y), Color.WhiteSmoke);
            }
        }
    }
}